...
prior_bm_mod <- c(prior(student_t(7, 0, 2), class = b),
                  ...
                  prior(student_t(7, 0, 2), class = b, dpar = "hu"))# set t priors with 7 df,
...
ggsave(plot = Figure3, filename = "04_figures/Fig3.png")
...
draws <- draws |> separate_wider_delim(par, delim="u_", names=c("par", "variable"),
  ...
  mutate(par = factor(ifelse(is.na(par), "Biomass", "Survival"),
         ...
         variable = case_when(variable == "TreatmentW:soilcontrol" ~ "Sterile Soil:\n 100% WHC",
                              variable == "TreatmentW:soilconspecific" ~ "Conspecific Soil:\n 100% WHC",
                              variable == "TreatmentW" ~ "100% WHC",
                              variable == "soilcontrol" ~ "Sterile Soil",
                              variable == "soilconspecific" ~ "Conspecific Soil",
...
draws$variable <- factor(draws$variable,levels = c("Cohort",
                                                   "Duration",
                                                   "Initial Height",
                                                   "Geochemistry",
                                                   "Conspecific Soil:\n 100% WHC",
...
draws_var$vital_rate <- factor(draws_var$vital_rate, levels = c("Survival", "Biomass"))
...
ggsave(plot = Figure2, filename = "04_figures/Fig2.png")
...
#version 1 ratios ----
brms_bmrat.mod <- brm(log(Root_Mass/Stem_Mass) ~ scale(duration) +
                        scale(Init_Height) + soil.dim1 + 
                        Treatment * soil + 
                        (1|Seedling) + (1|Inoc_Tag) + (1|Table) + (1|begin),
...
summary(brms_bmrat.mod)

#version 2 root mass fraction ---
...
plot_dat_RMF$soil <- factor(plot_dat_RMF$soil, levels = c("Sterile", "Conspecific", "Heterospecific"))

 ggplot(plot_dat_RMF, aes(x = Treatment, y=est, fill = soil)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(mapping = aes( ymin = mu.lower, ymax = mu.upper), width = 0.2,
                position = position_dodge(0.9)) +
  labs(x = "Soil Moisture Treatment", y = "Root Mass Fraction") +
  scale_x_discrete(labels = c("D" = "50% WHC", "W" = "100% WHC")) +
  scale_fill_viridis_d("") +
  theme_classic(12)

 Figure4 <- ggplot(filter(plot_dat_RMF, soil != "Sterile"), aes(x = soil, y=est)) +
...
ggsave(plot = Figure4, filename = "04_figures/Fig4.png")
...
ggsave(plot= Figure5, filename = "04_figures/Fig5.png")
